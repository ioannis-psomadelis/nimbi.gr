...
import { useSuspenseQuery, HydrationBoundary, type DehydratedState } from '@tanstack/react-query'
...
import { RouteContextProvider, type Language } from '../lib/route-context'
...
function HeaderSidebarTrigger() {
  ...
  const buttonClasses = "w-9 h-9 rounded-xl bg-muted/50 hover:bg-muted text-muted-foreground hover:text-foreground flex items-center justify-center"

  // SSR: render with desktop-hidden classes
  if (isMobile === undefined) {
    return (
      <div className="w-9 h-9 md:w-0 md:opacity-0 overflow-hidden transition-all duration-300 ease-out flex items-center">
  ...
  // Desktop: animate based on sidebar state
  return (
    <div
      className={`
        h-9 overflow-hidden transition-all duration-300 ease-out flex items-center
...
function ObservatoryContent({
  ...
  // Get weather data from React Query cache
  ...
  // Update selected run when model changes
  useEffect(() => {
  ...
  if (!proMode) {
    return (
      <SidebarProvider defaultOpen={true}>
        <Sidebar collapsible="offcanvas" className="!top-14 !bottom-9 !h-auto">
              ...
              <SidebarGroupContent>
                <div className="relative overflow-hidden rounded-xl bg-muted/50 border border-border p-4">
                  ...
                  <div className="relative">
                    ...
                    {nameLocal && (
                      <p className="text-sm text-muted-foreground mb-1">{nameLocal}</p>
                    )}
        ...
        <SidebarInset className="pt-14 pb-10 bg-background">
